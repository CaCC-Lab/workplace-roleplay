...
class HealthMetrics:
    ...
    def add_metric(self, category: str, name: str, value: any, status: str = "ok"):
        ...
        self.metrics[category][name] = {"value": value, "status": status, "timestamp": time.time()}
...
class ApplicationHealthChecker:
    ...
    async def check_service_status(self) -> Tuple[Dict, str]:
        ...
        try:
            result = subprocess.run(["systemctl", "is-active", "workplace-app"], capture_output=True, text=True)

            is_active = result.stdout.strip() == "active"

            # Get detailed service info
            result_status = subprocess.run(
                ["systemctl", "status", "workplace-app", "--no-pager", "-l"], capture_output=True, text=True
            )
    ...
    async def check_http_endpoints(self) -> Tuple[Dict, str]:
        ...
        endpoints = {"health": "/health", "api_models": "/api/models", "main_page": "/", "scenarios": "/scenarios"}
        ...
        async with aiohttp.ClientSession() as session:
            for name, endpoint in endpoints.items():
                try:
                    ...
                    results[name] = {"url": f"{self.base_url}{endpoint}", "error": "timeout", "healthy": False}
                    overall_status = "failed"

                except Exception as e:
                    results[name] = {"url": f"{self.base_url}{endpoint}", "error": str(e), "healthy": False}
                    overall_status = "failed"
    ...
    async def check_application_logs(self) -> Tuple[Dict, str]:
        ...
        log_paths = ["/opt/workplace-app/shared/logs/error.log", "/var/log/nginx/workplace-roleplay-error.log"]
        ...
        for log_path in log_paths:
            try:
                ...
                result = subprocess.run(["tail", "-100", str(log_file)], capture_output=True, text=True)
                ...
                log_analysis[log_path] = {"exists": False, "error": str(e)}
    ...
    if args.output == "json":
        ...
        for category, metrics in report["metrics"].items():
            ...
            for metric_name, metric_data in metrics.items():
                status_emoji = {"ok": "✅", "warning": "⚠️", "failed": "❌"}.get(metric_data["status"], "❓")
