...
class AtomicDeployer:
    ...
    async def _check_disk_space(self) -> bool:
        ...
        try:
            async with asyncssh.connect(
                ...
                result = await conn.run(f"df -h {self.config['app']['path']} | tail -1 | awk '{{print $5}}' | sed 's/%//'")
    ...
    async def _upload_application(self):
        ...
        exclude_patterns = [
            ...
            '*.log', '.env.local', 'deploy_key',
            '.pytest_cache', 'htmlcov', '*.egg-info'
        ]
