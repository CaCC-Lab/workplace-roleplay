...
def setup_logging(
    ...
    app_handler = RotatingFileHandler(app_log_path, maxBytes=max_bytes, backupCount=backup_count, encoding="utf-8")
    ...
    error_handler = TimedRotatingFileHandler(
        error_log_path, when="midnight", interval=1, backupCount=30, encoding="utf-8"
    )
    ...
    access_handler = TimedRotatingFileHandler(
        access_log_path, when="midnight", interval=1, backupCount=7, encoding="utf-8"
    )
...
def log_request_info(response):
    ...
    logger.info(f"{request.method} {request.path} - {response.status_code}", extra={"extra_data": log_data})
...
def log_exception(error: Exception, context: Optional[Dict[str, Any]] = None):
    ...
    logger.error(
        f"Exception occurred: {type(error).__name__}: {str(error)}", exc_info=True, extra={"extra_data": log_data}
    )
