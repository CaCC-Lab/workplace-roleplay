...
STRENGTH_CATEGORIES = {
    "empathy": {
        ...
        "indicators": ["相手の感情に配慮した返答", "相手の立場を理解する発言", "励ましや慰めの言葉"],
    },
    "clarity": {"name": "明確な伝達力", "description": "分かりやすく論理的に説明する力", "indicators": ["構造化された説明", "具体例の使用", "要点の明確化"]},
    "active_listening": {
        "name": "傾聴力",
        "description": "相手の話を真摯に聞き、理解を示す力",
        "indicators": ["相手の発言への適切な反応", "質問による理解の深化", "話の要約や確認"],
    },
    "adaptability": {
        "name": "適応力",
        "description": "相手や状況に応じて対応を調整する力",
        "indicators": ["相手に合わせた言葉遣い", "状況に応じた話題選択", "柔軟な対応"],
    },
    "positivity": {
        "name": "前向きさ",
        "description": "建設的で前向きな雰囲気を作る力",
        "indicators": ["ポジティブな表現", "解決志向の発言", "励ましや感謝の言葉"],
    },
    "professionalism": {
        "name": "プロフェッショナリズム",
        "description": "職場での適切な振る舞いと判断力",
        "indicators": ["適切な敬語使用", "ビジネスマナーの実践", "責任感のある対応"],
    },
}
...
ENCOURAGEMENT_PATTERNS = {
    ...
    "consistent_practice": {
        ...
        "threshold": 5,
        "messages": ["{count}回の練習を達成！継続は力なりです！", "コンスタントな練習が実を結んでいます。{top_skill}が特に光っています！", "毎日の積み重ねが確実な成長につながっています！"],
    },
    "strength_discovery": {
...
def create_strength_analysis_prompt(conversation_history: List[Dict[str, str]]) -> str:
    ...
    for key, category in STRENGTH_CATEGORIES.items():
        ...
        categories_description.append(f'{key}: {category["name"]} - {category["description"]}\n    - {indicators}')
...
def calculate_score_improvement(current_scores: Dict[str, float], history: List[Dict[str, Any]]) -> Dict[str, float]:
    """
    前回からのスコアの改善を計算
    """
    if not history:
...
def generate_encouragement_messages(scores: Dict[str, float], history: List[Dict[str, Any]]) -> List[str]:
    """
    ...
    if practice_count >= ENCOURAGEMENT_PATTERNS["consistent_practice"]["threshold"]:
        ...
        if top_skill:
            ...
            messages.append(message.format(count=practice_count, top_skill=top_skill["name"]))
        ...
        for key, diff in improvements.items():
            if diff >= ENCOURAGEMENT_PATTERNS["significant_growth"]["threshold"]:
                if key in STRENGTH_CATEGORIES:
                    ...
                    messages.append(message.format(skill=STRENGTH_CATEGORIES[key]["name"], diff=int(diff)))
                    break  # 1つのメッセージに留める
...
def create_personalized_message_prompt(scores: Dict[str, float], base_message: str) -> str:
    """
