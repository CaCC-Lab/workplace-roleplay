...
返答は自然で簡潔にし、相手が話しやすい雰囲気を作ってください。"""
    ...
    async def process_scenario_message(
        self, scenario_id: str, message: str, model_name: Optional[str] = None
...
- 純粋な会話文のみを返してください"""
    ...
    async def start_watch_conversation(
        self, topic: str, model1: Optional[str] = None, model2: Optional[str] = None
    ) -> AsyncGenerator[str, None]:
        """
        観戦モードで会話を開始
...
返答は簡潔にし、会話が続くような内容にしてください。"""
        ...
        # 結果を返す
        yield json.dumps(
            {"speaker": "model1", "model": model1, "message": model1_message},
            ensure_ascii=False,
        )
    ...
    async def continue_watch_conversation(self) -> AsyncGenerator[str, None]:
        ...
        if not topic or not history:
            yield json.dumps({"error": "会話が開始されていません"}, ensure_ascii=False)
            return
...
返答は簡潔にし、会話が続くような内容にしてください。"""
        ...
        # 結果を返す
        yield json.dumps(
            {"speaker": speaker, "model": next_model, "message": response_message},
            ensure_ascii=False,
        )
