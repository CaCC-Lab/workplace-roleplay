...
| URL | 画面 | 主要機能 |
|-----|------|---------|
| `/auth/login` | 利用開始 | 匿名サインイン（ワンクリック） |
...
| `/strength_analysis` | 強み分析 | レーダーチャート、6軸スキル可視化 |
| `/journal` | 学習ジャーナル | 学習履歴の振り返り（要: ENABLE_LEARNING_HISTORY=true） |
...
| supabase-py | 2.x | DB・認証クライアント |

### データベース・認証
| 技術 | 用途 |
|-----|------|
| Supabase (PostgreSQL) | ユーザーデータ・会話履歴の永続化 |
| Supabase Auth | 匿名サインイン（PIIなし） |
| Row Level Security | ユーザーごとのデータ分離 |
| JSONファイル | Supabase未設定時のフォールバック |
