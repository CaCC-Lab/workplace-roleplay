...
- ✅ **コマンドインジェクション対策**: subprocess使用を安全な代替手段に置き換え（2025年7月28日）
...
本アプリケーションを安全に運用するため、以下の環境変数の設定が必須です：
...
2. **必須環境変数の設定**
   - `FLASK_SECRET_KEY`: セッション暗号化用の秘密鍵
     ```bash
     # 安全な秘密鍵の生成
     python scripts/generate_secret_key.py
     ```
   - `GOOGLE_API_KEY`: Google Gemini APIキー（[取得方法](https://aistudio.google.com/)）

3. **セキュリティベストプラクティス**
   - 詳細は[SECURITY_BEST_PRACTICES.md](SECURITY_BEST_PRACTICES.md)を参照
   - 本番環境では64文字以上の秘密鍵を使用
   - `.env`ファイルは絶対にGitにコミットしない

### セキュリティ機能

- **自動的な脆弱キー検出**: 弱い秘密鍵は自動的に拒否
- **環境別設定**: 開発・本番環境で異なるセキュリティレベル
- **包括的な入力検証**: XSS、SQLインジェクション対策実装済み
