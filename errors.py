...
class AppError(Exception):
    ...
    def to_dict(self, include_details: bool = True) -> Dict[str, Any]:
        """エラーを辞書形式に変換"""
        error_dict = {"error": {"message": self.message, "code": self.code}}
...
class ExternalAPIError(AppError):
    ...
    def __init__(self, service: str, message: str, original_error: Optional[str] = None):
        ...
        super().__init__(
            message=f"{service}との通信でエラーが発生しました: {message}", code="EXTERNAL_API_ERROR", status_code=503, details=details
        )
...
class TimeoutError(AppError):
    ...
    def __init__(self, operation: str, timeout_seconds: Optional[int] = None):
        ...
        super().__init__(message=message, code="OPERATION_TIMEOUT", status_code=504, details=details)
...
class LLMError(AppError):
    ...
    def __init__(
        ...
        super().__init__(message=message, code="LLM_ERROR", status_code=503, details=details)
...
def handle_error(error: Exception) -> tuple:
    ...
    if is_development:
        ...
        error_response = {"error": {"message": "内部エラーが発生しました。しばらくしてから再試行してください。", "code": "INTERNAL_SERVER_ERROR"}}
...
def handle_llm_specific_error(error: Exception, model_name: str = "Gemini") -> AppError:
    ...
    if any(keyword in error_str for keyword in ["api key", "unauthorized", "401"]):
        return ExternalAPIError(service=model_name, message="APIキーが無効です", original_error=str(error))

    # その他のAPIエラー
    return ExternalAPIError(service=model_name, message="API呼び出しに失敗しました", original_error=str(error))
