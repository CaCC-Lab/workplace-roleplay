...
### 5.2 ビジュアル機能（新規提案：2025年1月9日）
- [ ] **AIキャラクター画像生成**
  - [ ] Gemini Image Generation API (gemini-2.0-flash-preview-image-generation) の統合
  - [ ] セリフに合わせた相手役の表情画像生成
  - [ ] シナリオのキャラクター設定に基づいた一貫性のあるビジュアル
  - [ ] 感情検出結果を表情に反映（喜び、悲しみ、怒り、困惑など）
  - [ ] 職場環境を背景とした画像生成
  - [ ] 生成画像のキャッシュ機能（同じ感情・キャラクターの再利用）
  
- [ ] **画像表示機能の実装**
  - [ ] 会話画面への画像表示エリアの追加
  - [ ] 画像の適切なサイズ調整とレイアウト
  - [ ] 画像生成中のローディング表示
  - [ ] 画像生成エラー時のフォールバック（デフォルト画像）
  
- [ ] **画像生成プロンプトの最適化**
  - [ ] キャラクター設定（年齢、性別、役職）の視覚化
  - [ ] 日本のビジネスシーンに適した服装・外見
  - [ ] 表情と感情の対応マッピング
  - [ ] シナリオごとの背景設定（会議室、オフィス、休憩室など）
