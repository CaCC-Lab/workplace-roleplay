...
class TestRegression:
    ...
    def test_basic_endpoints(self):
        ...
        basic_endpoints = [("GET /", None), ("GET /api/models", None), ("GET /api/tts/voices", None)]
    ...
    def test_chat_functionality(self):
        ...
        golden_cases = [
            tc
            for tc in self.golden_data["test_cases"]
            if tc["endpoint"] == "/api/chat" and tc.get("pattern", {}).get("name") == "simple_greeting"
        ]
        ...
        # 新しいメッセージを送信
        response = self.session.post(f"{self.base_url}/api/chat", json={"message": "こんにちは"}, headers=headers)
    ...
    def test_tts_functionality(self):
        """TTS機能のリグレッションテスト"""
        tts_cases = [tc for tc in self.golden_data["test_cases"] if tc["endpoint"] == "/api/tts"]

        if not tts_cases:
            pytest.skip("TTS機能のゴールデンデータが見つかりません")

        # 基本的なTTSテスト
        response = self.session.post(f"{self.base_url}/api/tts", json={"text": "こんにちは", "voice": "kore"})
    ...
    def _verify_endpoint(self, golden_case: Dict[str, Any]):
        ...
        if method == "GET":
            ...
            response = self.session.post(f"{self.base_url}{path}", json=golden_case.get("request"))
        else:
            pytest.skip(f"未対応のHTTPメソッド: {method}")

        # ステータスコードの確認
        assert response.status_code == golden_case["response_status"], (
            f"{endpoint} のステータスコードが異なります: " f"期待値={golden_case['response_status']}, 実際={response.status_code}"
        )
        ...
        if response.headers.get("content-type", "").startswith("application/json"):
            ...
            diff = DeepDiff(
                ...
                actual_data,
                ignore_order=True,
                exclude_paths=["root['timestamp']", "root['session_id']", "root['models']"],  # モデルリストは変更される可能性がある
            )
            ...
            if diff:
                ...
                assert not important_diff, f"{endpoint} のレスポンスに重要な違いがあります: {important_diff}"
...
class TestPerformanceRegression:
    ...
    def test_response_times(self):
        ...
        endpoints = [("GET", "/api/models"), ("GET", "/api/tts/voices")]
        ...
        for method, path in endpoints:
            ...
            assert elapsed_time < 1.0, f"{path} のレスポンスタイムが遅すぎます: {elapsed_time:.2f}秒"

            # ステータスコードも確認
            assert response.status_code in [200, 201], f"{path} が失敗しました: {response.status_code}"
