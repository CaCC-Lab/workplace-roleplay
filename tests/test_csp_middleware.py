...
class TestCSPMiddleware:
    ...
    def test_csp_report_endpoint(self, app):
        ...
        with app.test_client() as client:
            ...
            response = client.post("/api/csp-report", json=report, content_type="application/csp-report")
...
class TestCSPReportAnalyzer:
    ...
    def test_analyze_inline_script_violations(self):
        """インラインスクリプト違反の分析"""
        violations = [
            {"violated_directive": "script-src", "blocked_uri": "inline", "source_file": "/page1.html"},
            {"violated_directive": "script-src", "blocked_uri": "inline", "source_file": "/page2.html"},
        ]
        ...
        # インラインスクリプトの推奨事項を確認
        inline_rec = next((r for r in result["recommendations"] if "Inline scripts" in r["issue"]), None)
    ...
    def test_analyze_external_resource_violations(self):
        ...
        # 外部リソースの推奨事項を確認
        external_recs = [r for r in result["recommendations"] if "External resource blocked" in r["issue"]]
        assert len(external_recs) == 2  # cdn.example.comとfonts.example.com

    def test_analyze_eval_violations(self):
        """eval使用違反の分析"""
        violations = [{"violated_directive": "script-src", "blocked_uri": "eval", "source_file": "/app.js"}]

        result = CSPReportAnalyzer.analyze_violations(violations)

        # eval使用の推奨事項を確認
        eval_rec = next((r for r in result["recommendations"] if "eval()" in r["issue"]), None)
