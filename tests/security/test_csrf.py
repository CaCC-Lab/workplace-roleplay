...
class TestCSRFMiddleware:
    ...
    def test_require_csrf_decorator_valid_token(self):
        ...
        with app.test_client() as client:
            ...
            response = client.post("/protected", headers={"X-CSRF-Token": token}, json={"test": "data"})
    ...
    def test_require_csrf_decorator_invalid_token(self):
        ...
        with app.test_client() as client:
            ...
            # 無効なトークンでリクエスト
            response = client.post("/protected", headers={"X-CSRFToken": "invalid_token"})
    ...
    def test_require_csrf_decorator_with_form_data(self):
        ...
        with app.test_client() as client:
            ...
            # フォームデータにトークンを含める
            response = client.post("/protected", data={"csrf_token": token, "test": "data"})
            assert response.status_code == 200
