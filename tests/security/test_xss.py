...
class TestXSSPrevention:
    ...
    def test_JavaScriptプロトコルの無害化(self, csrf_client):
        ...
        payloads = ["<a href=\"javascript:alert('XSS')\">Click</a>", "<img src=\"javascript:alert('XSS')\">"]
        ...
        for payload in payloads:
            ...
            response = csrf_client.post(
                "/api/scenario_chat",
                json={"message": payload, "scenario_id": "scenario1", "model": "gemini-1.5-flash"},  # 実在するシナリオID
    ...
    def test_HTMLエンティティのエスケープ(self, csrf_client):
        ...
        for char, escaped in test_cases:
            ...
            with patch("app.initialize_llm") as mock_init_llm:
                ...
                response = csrf_client.post(
                    "/api/chat", json={"message": message}, headers={"X-CSRF-Token": csrf_token}
                )
    ...
    def test_エラーメッセージのサニタイズ(self, csrf_client):
        ...
        response = csrf_client.post(
            "/api/scenario_chat", json={"message": "test", "scenario_id": payload}, headers={"X-CSRF-Token": csrf_token}
        )
    ...
    def test_デバッグ情報の非表示(self, csrf_client):
        ...
        response = csrf_client.post("/api/chat", json={}, headers={"X-CSRF-Token": csrf_token})
...
class TestSecurityUtilsSanitization:
    ...
    def test_validate_model_name_rejects_invalid(self):
        ...
        invalid_names = ["../etc/passwd", "; DROP TABLE;", "<script>alert(1)</script>", "unknown-model-name"]
    ...
    def test_validate_model_name_accepts_valid(self):
        ...
        valid_names = ["gemini-1.5-pro", "gemini-1.5-flash", "gemini/gemini-2.0-flash", "gemini-2.5-flash-lite"]
