...
class TestChatAPI:
    ...
    def test_チャットAPIが正常に応答を返す(self, csrf_client):
        ...
        with patch("app.initialize_llm") as mock_init_llm:
            ...
            with csrf_client.session_transaction() as sess:
                sess["chat_settings"] = {"system_prompt": "あなたは職場での雑談練習をサポートするAIアシスタントです。", "model": "gemini-1.5-flash"}

            # リクエストを送信
            response = csrf_client.post("/api/chat", json={"message": "こんにちは", "model": "gemini-1.5-flash"})
    ...
    def test_会話履歴がセッションに保存される(self, csrf_client):
        ...
        with patch("app.initialize_llm") as mock_init_llm:
            ...
            # チャットセッションを初期化
            with csrf_client.session_transaction() as sess:
                sess["chat_settings"] = {"system_prompt": "テスト用プロンプト", "model": "gemini-1.5-flash"}

            # 最初のメッセージ - レスポンスを消費
            response = csrf_client.post("/api/chat", json={"message": "テスト1", "model": "gemini-1.5-flash"})
            # ストリーミングレスポンスを消費
...
class TestScenarioAPI:
    ...
    def test_シナリオチャットが正常に動作する(self, csrf_client):
        ...
        with patch("app.initialize_llm") as mock_init_llm:
            ...
            response = csrf_client.post(
                "/api/scenario_chat", json={"message": "こんにちは", "model": "gemini-1.5-flash", "scenario_id": "scenario1"}
            )
    ...
    def test_存在しないシナリオIDはエラーを返す(self, csrf_client):
        ...
        response = csrf_client.post(
            "/api/scenario_chat",
            json={"message": "テスト", "model_id": "gemini-1.5-flash", "scenario_id": "invalid_scenario_999"},
        )
...
"""
            ...
            response = csrf_client.post("/api/scenario_feedback", json={"scenario_id": "scenario1"})
    ...
    def test_観戦モード次の会話が生成される(self, csrf_client):
        ...
        with patch("app.initialize_llm") as mock_init_llm:
            ...
            with csrf_client.session_transaction() as sess:
                ...
                sess["watch_history"] = [{"speaker": "A", "message": "こんにちは"}]
...
class TestErrorHandling:
    ...
    def test_不正なJSONリクエストはエラーを返す(self, csrf_client):
        ...
        response = csrf_client.post("/api/chat", data="不正なJSON", content_type="application/json")
...
class TestModelSelection:
    ...
    def test_デフォルトモデルが使用される(self, csrf_client):
        ...
        with patch("app.initialize_llm") as mock_init_llm:
            ...
            response = csrf_client.post("/api/chat", json={"message": "テスト"})
