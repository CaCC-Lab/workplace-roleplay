...
class TestChatService:
    ...
    def setup_method(self):
        ...
        # ChatServiceインスタンス
        self.service = ChatService(llm_service=self.mock_llm_service, session_service=self.mock_session_service)
    ...
    async def test_process_chat_message_success(self):
        ...
        self.mock_session_service.add_chat_message.assert_called_once_with("こんにちは", "こんにちは！お元気ですか？")
    ...
    async def test_process_scenario_message_success(self):
        ...
        mock_scenario = {
            ...
            "title": "テストシナリオ",
            "situation": "テスト状況",
            "character": {"name": "田中さん", "role": "上司", "personality": "優しくて理解がある"},
        }
    ...
    async def test_generate_scenario_feedback(self):
        ...
        mock_scenario = {
            ...
            "title": "テストシナリオ",
            "situation": "テスト状況",
            "character": {"name": "田中さん", "role": "上司"},
            "feedback_points": ["明確な報告", "適切な敬語"],
        }

        with patch("services.chat_service.get_scenario_by_id", return_value=mock_scenario):
            # モック設定
            self.mock_session_service.get_scenario_history.return_value = [{"human": "報告があります", "ai": "はい、聞きます"}]
