...
class TestBuildChatFeedbackPrompt:
    ...
    def test_returns_string(self, feedback_service, sample_history):
        ...
        result = feedback_service.build_chat_feedback_prompt(
            sample_history,
            partner_type="同僚",
            situation="朝の挨拶"
        )
...
class TestBuildScenarioFeedbackPrompt:
    ...
    @patch('services.feedback_service.get_scenario_service')
    def test_returns_string_reverse_role(self, mock_get_service, feedback_service, sample_history, sample_scenario_data):
    ...
    @patch('services.feedback_service.get_scenario_service')
    def test_reverse_role_contains_harassment_prevention(self, mock_get_service, feedback_service, sample_history, sample_scenario_data):
...
class TestTryMultipleModelsForPrompt:
    ...
    @patch('google.generativeai.configure')
    @patch('app.create_model_and_get_response')
    def test_returns_content_on_success(self, mock_create, mock_configure, mock_list_models, mock_config, feedback_service):
    ...
    @patch('google.generativeai.configure')
    @patch('app.create_model_and_get_response')
    def test_handles_rate_limit_error(self, mock_create, mock_configure, mock_list_models, mock_config, feedback_service):
        """レート制限エラーのハンドリングテスト"""
...
class TestUpdateFeedbackWithStrengthAnalysis:
    ...
    def test_returns_dict(self, feedback_service, sample_feedback_response):
        """辞書が返されることをテスト"""
        result = feedback_service.update_feedback_with_strength_analysis(
