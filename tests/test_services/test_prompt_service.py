...
class TestBuildScenarioSystemPrompt:
    ...
    def sample_scenario(self):
        return {
            "title": "進捗報告シナリオ",
            "character": {"name": "田中課長", "role": "上司", "personality": "厳格だが公平な上司で、部下の成長を見守っている。"},
...
class TestBuildScenarioFeedbackPrompt:
    ...
    @pytest.fixture
    def sample_scenario(self):
        return {"title": "報告シナリオ", "situation": "進捗報告の場面", "your_role": "部下", "feedback_points": ["報告の明確さ", "課題の伝え方"]}

    def test_returns_string(self, sample_scenario):
        """文字列が返されることをテスト"""
        result = PromptService.build_scenario_feedback_prompt(sample_scenario, "あなた: 報告があります\n上司: はい、どうぞ")
        assert isinstance(result, str)
...
class TestFormatConversationForFeedback:
    ...
    def test_formats_history(self):
        ...
        history = [{"human": "こんにちは", "ai": "こんにちは！"}, {"human": "お元気ですか？", "ai": "はい、元気です！"}]
