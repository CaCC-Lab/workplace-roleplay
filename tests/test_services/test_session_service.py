...
class TestSessionService:
    ...
    def test_get_chat_history(self, mock_get_memory):
        """チャット履歴取得のテスト"""
        mock_get_memory.return_value = [{"human": "test1", "ai": "response1"}, {"human": "test2", "ai": "response2"}]
    ...
    def test_add_scenario_message(self, mock_add_history):
        """シナリオメッセージ追加のテスト"""
        self.service.add_scenario_message("scenario-001", "提案があります", "はい、お聞きします", "部下")
    ...
    def test_add_watch_message(self, mock_add_history):
        """観戦モードメッセージ追加のテスト"""
        self.service.add_watch_message("モデル1のメッセージ", "モデル2のメッセージ", "gemini-1.5-flash", "gemini-1.5-pro")
    ...
    def test_add_learning_record(self):
        ...
        self.service.add_learning_record(
            activity_type="scenario", scenario_id="scenario-001", feedback={"score": 85}, duration_seconds=300
        )
...
class TestSessionServiceExtended:
    ...
    def test_get_scenario_history_with_data(self):
        """シナリオ履歴がある場合"""
        self.mock_session[SessionKeys.SCENARIO_HISTORY] = {"scenario1": [{"human": "test", "ai": "response"}]}
    ...
    def test_clear_scenario_history_nonexistent(self):
        """存在しないシナリオの履歴をクリア"""
        self.mock_session[SessionKeys.SCENARIO_HISTORY] = {"scenario-001": [{"human": "test1"}]}
    ...
    def test_get_watch_history(self):
        """観戦モード履歴の取得"""
        self.mock_session[SessionKeys.WATCH_HISTORY] = [{"model1": {"name": "A", "message": "Hello"}}]
