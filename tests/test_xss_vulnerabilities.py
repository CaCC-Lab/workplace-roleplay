...
class TestXSSVulnerabilities:
    ...
    def test_watch_mode_xss(self, csrf_client):
        ...
        response = csrf_client.post("/api/watch/start", json={"topic": xss_topic, "model": "gemini-1.5-flash"})
    ...
    def test_stored_xss_in_history(self, csrf_client):
        ...
        with csrf_client.session_transaction() as sess:
            ...
            sess["chat_history"] = [{"human": '<script>alert("stored XSS")</script>', "ai": "Response"}]
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "/api/scenario_chat",
                "POST",
                {"message": '<iframe src="javascript:alert(1)">', "model": "gemini-1.5-flash"},
            ),
            (
                "/api/watch/start",
                "POST",
                {
                    "topic": '<embed src="data:text/html;base64,PHNjcmlwdD5hbGVydCgxKTwvc2NyaXB0Pg==">',
