...
class TestCSRFIntegration:
    ...
    def test_protected_endpoint_with_invalid_csrf_token(self, client):
        ...
        response = client.post("/api/clear_history", json={"mode": "chat"}, headers={"X-CSRFToken": "invalid_token"})
    ...
    def test_protected_endpoint_with_valid_csrf_token(self, client):
        ...
        # 有効なトークンでアクセス
        response = client.post("/api/clear_history", json={"mode": "chat"}, headers={"X-CSRFToken": csrf_token})
    ...
    def test_chat_endpoint_csrf_protection(self, mock_llm, client):
        ...
        response = client.post("/api/chat", json={"message": "こんにちは"}, headers={"X-CSRFToken": csrf_token})
    ...
    def test_multiple_csrf_protected_endpoints(self, client):
        ...
        for endpoint, data in protected_endpoints:
            ...
            response = client.post(endpoint, json=data, headers={"X-CSRFToken": csrf_token})
    ...
    def test_csrf_error_logging(self, client, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            ...
            response = client.post(
                "/api/clear_history", json={"mode": "chat"}, headers={"X-CSRFToken": "invalid_token"}
            )

            assert response.status_code == 403

            # ログが記録されていることを確認（caplogを使用）
            assert any("csrf" in record.message.lower() for record in caplog.records) or any(
                "CSRF" in record.message for record in caplog.records
            )
    ...
    def test_csrf_with_form_data(self, client):
        ...
        response = client.post("/api/clear_history", data={"mode": "chat", "csrf_token": csrf_token})
