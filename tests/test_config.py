...
def create_test_config_class(base_class):
    ...
    # 新しいクラスを動的に作成
    return type(class_name, (base_class,), {"model_config": new_model_config})
...
class TestConfigBase:
    ...
    def test_環境変数からの読み込み(self):
        ...
        with patch.dict(
            os.environ, {"FLASK_SECRET_KEY": "test-secret", "DEFAULT_TEMPERATURE": "0.9", "LOG_LEVEL": "DEBUG"}
        ):
            config = TestConfig()
    ...
    def test_型変換が正しく行われる(self):
        ...
        with patch.dict(
            os.environ, {"DEFAULT_TEMPERATURE": "0.5", "SESSION_LIFETIME_MINUTES": "60", "ENABLE_DEBUG": "true"}
        ):
            config = TestConfig()
    ...
    def test_無効な値のバリデーション(self):
        ...
        with patch.dict(os.environ, {"DEFAULT_TEMPERATURE": "1.5"}):  # 範囲外
            with pytest.raises(ValueError, match="Temperature must be between 0 and 1"):
                TestConfig()

        with patch.dict(os.environ, {"SESSION_LIFETIME_MINUTES": "-10"}):  # 負の値
    ...
    def test_必須環境変数のチェック(self):
        ...
        with patch.dict(os.environ, {"FLASK_SECRET_KEY": "prod-secret-key"}, clear=True):
...
class TestConfigValidation:
    ...
    def test_モデル名の検証(self):
        ...
        valid_models = ["gemini/gemini-1.5-pro", "gemini/gemini-1.5-flash"]
...
class TestConfigFile:
    ...
    def test_JSONファイルからの読み込み(self):
        ...
        config_data = {"DEFAULT_TEMPERATURE": 0.8, "LOG_LEVEL": "WARNING", "SESSION_LIFETIME_MINUTES": 45}
    ...
    def test_環境変数が設定ファイルより優先される(self):
        ...
        config_data = {"DEFAULT_TEMPERATURE": 0.8, "LOG_LEVEL": "WARNING"}
