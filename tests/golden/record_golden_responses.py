...
class GoldenTestRecorder:
    ...
    def record_chat_scenarios(self):
        ...
        test_patterns = [
            {"name": "simple_greeting", "messages": ["こんにちは"], "description": "シンプルな挨拶"},
            {
                "name": "multi_turn_conversation",
                "messages": ["こんにちは", "今日の天気はどうですか？", "ありがとうございます"],
                "description": "複数ターンの会話",
            },
            {"name": "long_message", "messages": ["これは長いメッセージのテストです。" * 50], "description": "長いメッセージ"},
        ]
        ...
        for pattern in test_patterns:
            ...
            for message in pattern["messages"]:
                ...
                response = self.session.post(f"{self.base_url}/api/chat", json=request_data)
    ...
    def record_scenario_flows(self):
        ...
        if response.status_code == 200:
            ...
            if scenarios:
                ...
                response = self.session.post(f"{self.base_url}/api/scenario/start", json=request_data)
                self.record_test_case("POST /api/scenario/start", request_data, response)

                # シナリオメッセージ
                if response.status_code == 200:
                    request_data = {"scenario_id": test_scenario["id"], "message": "はい、わかりました"}
                    response = self.session.post(f"{self.base_url}/api/scenario/message", json=request_data)
    ...
    def record_watch_mode(self):
        ...
        # 観戦開始
        request_data = {"theme": "新人研修での質問", "personas": {"A": "gemini-1.5-flash", "B": "gemini-1.5-pro"}}
        response = self.session.post(f"{self.base_url}/api/watch/start", json=request_data)
    ...
    def record_tts_functionality(self):
        ...
        test_cases = [
            {"text": "こんにちは", "voice": "kore", "description": "基本的なTTS"},
            {"text": "これはテストです", "voice": "charon", "emotion": "happy", "description": "感情付きTTS"},
        ]
        ...
        for test_case in test_cases:
            ...
            response = self.session.post(f"{self.base_url}/api/tts", json=request_data)
